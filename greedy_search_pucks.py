...
def _(
    ...
    # Container to store widget reference and loss function
    widget_ref = [None]
    ...
    def draw_loss_landscape(ax, x, y):
        """Draw the loss landscape and greedy search trajectory."""
        loss_fn = current_loss_fn[0]

        # Access puck position from the widget (this is the starting point)
        puck_widget = widget_ref[0]
        if puck_widget is not None:
            start_x = puck_widget.x[0]
            start_y = puck_widget.y[0]
        else:
            start_x = initial_x[0]
            start_y = initial_y[0]
