...
def _(mo):
    ...
    Reproducing the "repeat the prompt" technique: transform `<QUERY>` into `<QUERY><QUERY>`.
    We evaluate this on ARC-Easy multiple-choice science questions across models.
    """)
    return
...
def _():
    ...
    class ExperimentParams(BaseModel):
        ...
        repeat_count: int = Field(default=2, description="Number of times to repeat the prompt (2 or 3)")
        prompt_order: str = Field(default="question_first", description="'question_first' or 'options_first'")
...
def _(MCAnswer):
    ...
    def build_prompt(question: str, choices: dict, prompt_order: str) -> str:
        ...
        instruction = (
            f"Reply with ONLY the letter of the correct answer ({valid}). "
            "Do not explain."
        )
...
def _(
    ...
    async def _eval_one(ex, model, repeat_count):
        _cache_key = (base_url_input.value, model, ex["id"], repeat_count, params.prompt_order, params.structured_output)
        _cached = cache.get(_cache_key)
        if _cached is not None:
            return _cached

        prompt_text = build_prompt(ex["question"], ex["choices"], params.prompt_order) * repeat_count
    ...
    # Log to wandb
    cols = ["id", "model", "repeat_count", "question", "correct", "predicted", "raw_response", "match"]
